...
test('resolves urls', (t) => process("a { b: resolve('picture.png') }", {
...
test('resolves urls from the current path', (t) => process("a { b: resolve('picture.png') }", {
  basePath: 'test/fixtures',
  baseUrl: 'http://example.com/wp-content/themes',
}, {
...
test('resolves relative urls from the current path', (t) => process("a { b: resolve('fonts/empty-sans.woff') }", {
  basePath: 'test/fixtures',
  relative: true,
}, {
  from: path.resolve('test/fixtures/images/style.css'),
...
test('resolves relative urls from the provided path', (t) => process("a { b: resolve('fonts/empty-sans.woff') }", {
  basePath: 'test/fixtures',
  relative: 'fonts',
})
  .then((result) => {
...
test('busts cache when resolving urls', (t) => process("a { b: resolve('picture.png') }", {
  basePath: 'test/fixtures',
  baseUrl: 'http://example.com/wp-content/themes',
...
test('throws when trying to resolve a non-existing file', (t) => process("a { b: resolve('non-existing.gif') }")
  .then(t.fail, (err) => {
    t.true(err instanceof Error);
    t.is(err.message, 'Asset not found or unreadable: non-existing.gif');
  }));

test('inlines data', (t) => process("a { b: inline('picture.png') }", {
  basePath: 'test/fixtures',
  loadPaths: ['fonts', 'images'],
})
  .then((result) => {
...
test('inlines svg unencoded', (t) => process("a { b: inline('vector.svg') }", {
  basePath: 'test/fixtures',
  loadPaths: ['fonts', 'images'],
})
  .then((result) => {
...
test('throws when trying to inline a non-existing file', (t) => process("a { b: inline('non-existing.gif') }")
  .then(t.fail, (err) => {
    t.true(err instanceof Error);
    t.is(err.message, 'Asset not found or unreadable: non-existing.gif');
  }));

test('measures images', (t) => process("a { "
    + "b: size('vector.svg'); "
    + "c: width('picture.png'); "
...
test('measures images with density provided', (t) => process("a { "
    + "b: size('vector.svg', 2); "
...
test('throws when trying to measure a non-existing image', (t) => process("a { b: size('non-existing.gif') }")
  .then(t.fail, (err) => {
    t.true(err instanceof Error);
    t.is(err.message, 'Asset not found or unreadable: non-existing.gif');
  }));

test('throws when trying to measure an unsupported file', (t) => process("a { b: size('test/fixtures/fonts/empty-sans.woff') }")
  .then(t.fail, (err) => {
...
test('throws when trying to measure an invalid file', (t) => process("a { b: size('test/fixtures/images/invalid.jpg') }")
  .then(t.fail, (err) => {
...
test('handles quotes and escaped characters', (t) => process("a {"
...
test('allows usage inside media queries', (t) => process("@media a and (b: height('test/fixtures/images/picture.png')) { c { d: e }}")
  .then((result) => {
